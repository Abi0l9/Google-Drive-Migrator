...
import { pauseMigrationForUser } from "@/lib/migration/controls";
...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await pauseMigrationForUser(user._id.toString(), id);
  if (result.outcome === "not_found") {
    return NextResponse.json({ error: "Migration not found" }, { status: 404 });
  }
  if (result.outcome === "conflict") {
    return NextResponse.json({ error: `A ${result.status} migration cannot be paused` }, { status: 409 });
  }
